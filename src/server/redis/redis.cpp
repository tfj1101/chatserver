...
{
    auto reply = (redisReply *)redisCommand(_publish_context, "PUBLISH %d %s", channel, message.c_str());
    if (reply == nullptr)
    {
        printf("Error: %s\n", _publish_context->errstr);
        return false;
    }
...
bool Redis::subscribe(int channel)
{
    ...
    // redisBufferWrite可以循环发送缓冲区，知道缓冲区数据发送完毕（done置为1）
    int done = 0;
    while (!done)
    {
